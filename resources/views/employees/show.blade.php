<x-app-layout>
    ...
    <div class="flex mx-auto mt-8 space-x-3 max-w-7xl">
        ...
        <div class="w-3/4 p-5 bg-white rounded-md shadow">
            ...
            <h3><strong>Employee No.: </strong>{{ $employee->emp_no }}</h3>
            ...
            <div class="w-2/4">
                ...
                @forelse ($employee->allowances as $allowance)
                    <span>{{ $allowance->allowance->allowance_name }} - {{ $allowance->allowance->allowance_amount }}</span>
                @empty
            ...
            $total_deductions = $employee->computeDeduction();
                ...
                @forelse ($employee->deductions as $deduction)
                    <span>{{ $deduction->deduction->deduction_name }} - {{ $deduction->deduction->deduction_amount_type == 'percentage' ? percentage($deduction->deduction->deduction_amount) : number_format($deduction->deduction->deduction_amount) }}</span>
                    <br>
                    @empty
