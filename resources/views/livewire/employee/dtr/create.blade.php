            ...
            <div class="flex flex-col">
                <div class="mb-6">
                    <div class="flex justify-between items-center">
                        ...
                        <img src="{{ asset('assets/images/cl-pnp-logo.png') }}" alt="cl pnp logo"
                        style="height: 70px; width:70px;">
                    </div>
                    <h2 class="text-sm font-bold text-center mt-3">DAILY TIME RECORD</h2>
                    <p class="text-xs text-start mt-3">Name:
                        <span class="font-bold border-b border-black">
                            {{ $employee->full_name }}
                        </span>
                    </p>

                    <p class="text-xs text-start mt-3">For the month of
                        <span class="font-bold border-b border-black">
                            {{ Str::upper(date('F', strtotime($selected_month))) }}
                        </span>
                    </p>

                    <p class="text-xs text-center mt-3">Official hours for arrival and departure</p>
                </div>
                    ...
                    <table class="border border-black w-full">
                        ...
                        <tbody>
                            ...
                            <tr style="background-color:{{ $attendance['color'] }}"
                                ...
                                <td class="border border-black text-center py-2 text-[10px]">{{ $attendance['day'] }}
                                </td>
                                <td class="border border-black text-center py-2 text-[10px]">{{ $attendance['time_in'] ?
                                    date('h:i A', strtotime($attendance['time_in'])) : '---------' }}</td>
                                <td class="border border-black text-center py-2 text-[10px]">{{ $attendance['time_out']
                                    ?
                                    date('h:i A', strtotime($attendance['time_out'])) : '---------' }}</td>
                                <td class="border border-black text-center py-2 text-[10px]">
                                    {{ $attendance['under_time_hours'] != 0 ? "{$attendance['under_time_hours']} Hrs" :
                                    '---------' }}
                                </td>
                                <td class="border border-black text-center py-2 text-[10px]">
                                    {{ $attendance['under_time_minutes'] != 0 ? "{$attendance['under_time_minutes']}
                                    Mins" :
                                    '---------' }}
                                </td>
                            </tr>
