<div class="overflow-hidden sm:rounded-md">
    <form wire:submit.prevent="save" enctype="multipart/form-data">
        {{-- <form action="{{ route('employees.store') }}" method="POST" enctype="multipart/form-data"> --}}
            @csrf
            @if ($errors->any())
                <div class="bg-red-500 text-white rounded px-3 py-5 mb-3">
                    <ul>
                        @foreach ($errors->all() as $error)
                            <li><span class="font-bold">{{ $loop->iteration }}</span>. {{ $error }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-2">
                        <label for="salary_grade_step" class="block font-medium text-gray-700">Salary
                            Grade Step</label>
                        <select name="salary_grade_step" id="salary_grade_step" wire:model.live='salary_grade_step'
                            class="block w-full mt-1 rounded form-select" required>
            ...
            <div class="px-4 py-5 my-3 bg-white shadow sm:p-6">
                    ...
                    <div class="col-span-6 sm:col-span-2">
                        ...
                        <div class="flex flex-col">
                            ...
                            <div class="flex items-center mt-1" wire:key="{{ $allowance->id }}">
                                ...
                                <label for="allowance_{{ $allowance->id }}" class="text-gray-900">{{
                                    $allowance->allowance_code }}</label>
                            </div>
                            @empty
                    ...
                    <div class="col-span-6 sm:col-span-2">
                        ...
                        <div class="flex flex-wrap">
                            ...
                            <div class="w-1/2 px-2" wire:key="{{ $mandatory_deduction->id }}">
                                <input type="checkbox" name="deductions[]" value="{{ $mandatory_deduction->id }}"
                                    class="hidden mr-2 form-checkbox"
                                    wire:model="selectedMandatoryDeductionIds.{{ $mandatory_deduction->id }}" checked>
                                ...
                                <label for="deduction_{{ $mandatory_deduction->id }}" class="text-gray-900">{{
                                    $mandatory_deduction->deduction_code }}</label>
                            </div>
                            @empty
                        ...
                        <div class="flex flex-wrap mt-4">
                            ...
                            <div class="w-1/2 px-2" wire:key="{{ $non_mandatory_deduction->id }}">
                                ...
                                <label for="deduction_{{ $non_mandatory_deduction->id }}" class="text-gray-900">{{
                                    $non_mandatory_deduction->deduction_code }}</label>
                            </div>
                            @empty
                            <p>No deduction found.</p>
                            @endforelse
                ...
                <div class="grid grid-cols-6 gap-6 mt-3" wire:key="{{ $selected_loan->id }}">
                    <div class="col-span-3 sm:col-span-2">
                        ...
                        <div>
                            <label for="amount" class="block font-medium text-gray-700">{{ $selected_loan->name }}
                            </label>
                            <div class="flex flex-col">
                            ...
                            <div class="flex flex-col w-1/2 space-y-2">
                                <div class="w-1/2 px-2">
                                    <input type="checkbox" name="1-15_{{ $selected_loan->id }}"
                                        id="1-15_{{ $selected_loan->id }}" value="1-15" class="mr-2 form-checkbox"
                                ...
                                <div class="w-1/2 px-2">
                                    <input type="checkbox" name="16-31_{{ $selected_loan->id }}"
                                        id="16-31_{{ $selected_loan->id }}" value="16-31" class="mr-2 form-checkbox"
