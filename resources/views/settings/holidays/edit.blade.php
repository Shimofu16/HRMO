<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Holiday: {{ date('M d, Y', strtotime($holiday->date)) }}
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('holidays.update', $holiday) }}" method="POST">
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <input type="text" name="name" id="name" class="form-input mt-1 block w-full text-xl" value="{{ $holiday->name }}" required>
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <label for="date" class="block font-medium text-gray-700">Date</label>
                                <input type="date" name="date" id="date" class="form-input mt-1 block w-full text-xl" value="{{ $holiday->date }}" required>
                            </div>
                        </div>
